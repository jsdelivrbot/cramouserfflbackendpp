...
router.post('/lots', function(req, res) {
  res.json(updateLot({lot: req.body.lot}))
...
router.get('/addLot/:r/:c', function(req, res) {
  if(isLetter(req.params.r) && !isNaN(Number(req.params.c))) {
    for(let l of lots) {
      if(l.col == req.params.c && l.row === req.params.r) {
        return res.json({success: true})    
      }
    }
    lots.push (
      {
        id:req.params.c + req.params.r,
        col: Number(req.params.c),
        row: req.params.r,
        floors: [{},{},{}]
      }
    )
...
router.get('/removeLot/:r/:c', function(req, res) {
  if(isLetter(req.params.r) && !isNaN(Number(req.params.c))) {
    for(let i=0;i<lots.length;i++) {
      if(lots[i].col == req.params.c && lots[i].row === req.params.r) {
        lots.splice(i, 1)
        io.emit('update', { for: 'everyone' })
        return res.json({success: true})    
      }
